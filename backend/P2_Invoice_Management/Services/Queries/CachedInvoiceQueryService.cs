...
public class CachedInvoiceQueryService : IInvoiceQueryService
{
    ...
    {
        string cacheKey = $"invoices:all:{page}:{pageSize}";
        var cachedInvoices = await _cacheService.GetAsync<IEnumerable<Invoice>>(cacheKey);
        
        if (cachedInvoices != null)
            return cachedInvoices;

        var invoices = await _baseQueryService.GetAllInvoicesAsync(page, pageSize);
    ...
    public async Task<object> GetAgingReportAsync()
    {
        ...
        var data = await _baseQueryService.GetAgingReportAsync();
    ...
    {
        const string cacheKey = "analytics:revenue-summary";
        var cachedData = await _cacheService.GetAsync<object>(cacheKey);
        if (cachedData != null)
            return cachedData;

        var data = await _baseQueryService.GetRevenueSummaryAsync();
        await _cacheService.SetAsync(cacheKey, data, TimeSpan.FromMinutes(5));
        return data;
    }
    ...
    {
        string cacheKey = $"analytics:dso:{days}";
        var cachedData = await _cacheService.GetAsync<object>(cacheKey);
        if (cachedData != null)
            return cachedData;

        var data = await _baseQueryService.GetDsoAsync(days);
        await _cacheService.SetAsync(cacheKey, data, TimeSpan.FromMinutes(5));
        return data;
    }
    ...
    public async Task<object> GetOutstandingAnalyticsAsync()
    {
        ...
        var data = await _baseQueryService.GetOutstandingAnalyticsAsync();
        await _cacheService.SetAsync(cacheKey, data, TimeSpan.FromMinutes(5));
        return data;
    }
}
...
public interface IInvoiceQueryService
{
    ...
    Task<IEnumerable<Invoice>> GetAllInvoicesAsync(int page = 1, int pageSize = 20);
    ...
    Task<object> GetRevenueSummaryAsync();
    Task<object> GetDsoAsync(int days = 30);
    Task<object> GetOutstandingAnalyticsAsync();
}
